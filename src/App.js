...
class App extends React.Component {
  ...
  handleOnSubmit = (event) => {
    ...
    const newFoodArray = [{ name: foodName, calories: foodCalories, image: foodImage }, ...this.state.foodList];
    this.setState({
  ...
  render() {
    const totalCalories = this.state.todayFood.reduce((acc, element) => acc + element.calories, 0)
    return (
      <div className="App">
        ...
        { this.state.isFormDisplayed &&
          <form onSubmit={this.handleOnSubmit}>
            ...
            <input name="foodCalories" placeholder="23" value={this.state.foodCalories} onChange={this.handleOnChange}></input>
            <label> Image url</label>
            <input name="foodImage" placeholder="url" value={this.state.foodImage} onChange={this.handleOnChange}></input>
        ...
        <div className="food-container">
          <div>
            {this.state.foodList.filter(element => element.name.toLocaleLowerCase().includes(this.state.searchParams.toLocaleLowerCase())).map((element, idx) => <FoodBox key={idx} food={element} deleteFood={this.deleteFood} addQuantity={this.addQuantity} />)}
          </div>
          <div>
            <h1> Today's food</h1>
            <ul>
